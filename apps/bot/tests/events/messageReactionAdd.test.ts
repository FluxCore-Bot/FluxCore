...
const eventModule = await import("../../src/events/messageReactionAdd.js");
...
describe("messageReactionAdd event", () => {
  ...
  it("calls both role panel and starboard handlers", async () => {
    ...
    expect(mockHandleRolePanelReaction).toHaveBeenCalledWith(reaction, user, true);
  ...
  it("does not throw when one handler errors", async () => {
    mockHandleStarboardReaction.mockRejectedValueOnce(new Error("DB error"));
    const reaction = createMockReaction();
    const user = createMockUser();
